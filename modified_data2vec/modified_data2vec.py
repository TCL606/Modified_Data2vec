...
class Data2VecAudioModel(BaseFairseqModel):
    def __init__(self, cfg: Data2VecAudioConfig):
        ...
        self.n_negatives = 100 # 100 negative samples
        self.cross_sample_negatives = 0 # wav2vec is 0
        self.logit_temp = 0.1
        self.xla = False
    ...
    def apply_mask(
        ...
        if self.mask_channel_prob > 0 and self.mask_channel_before: # !!! when pretraining, m_c_p = 0.0
        ...
        if self.mask_prob > 0: #!!! when pretraining, m_p = 0.65
    ...
    def compute_preds(self, x, y, negatives):
        ...
        y = y.unsqueeze(0)
        targets = torch.cat([y, negatives], dim=0) # !!! shape[0] = 101 

        logits = torch.cosine_similarity(x.float(), targets.float(), dim=-1) # !!! cal the sim
    ...
    def forward(
        ...
        features = source #!!! raw audio

        if self.feature_grad_mult > 0:
            features = self.feature_extractor(features) # !!! BxCxL
        ...
        if mask:
            x, mask_indices = self.apply_mask( # x:[1,773,768], mask:[1,773]
                features,
                padding_mask,
        ...
        if not is_xla_tensor(x):
            ...
            x = x[mask_indices].view(x.size(0), -1, x.size(-1)) # x:[1,773,768]=>[1,413,768]

        if not is_xla_tensor(y):
            # tpu-comment: reducing the size in a dynamic way causes
            # too many recompilations on xla.
            y = y[mask_indices].view(y.size(0), -1, y.size(-1)) # y:[1,773,768]=>[1,413,768]
        ...
        cos_similarity = self.compute_preds(x, y, negs) # [101,1,413]
        
        logits = cos_similarity.transpose(0,2) # [413,1,101]
        logits = logits.reshape(-1, logits.size(-1)) # [413*batch, 101]
        logits = logits.float()

        target = cos_similarity.new_zeros(cos_similarity.size(1) * cos_similarity.size(2), dtype=torch.long) # [batch*413]
